...
1. Consultoría de licitaciones (servicio personalizado):
...
1. Mensajes cortos: máximo 3-4 oraciones. WhatsApp no es un email.
1. Si preguntan por un proceso específico, usa el CONTEXTO provisto — nunca inventes datos
1. Si no tienes info de un proceso, ofrece analizarlo en LicitacionLab
1. Si piden cotización del servicio de consultoría, di que el Ing. Luis Antigua los contactará personalmente
1. Cierra siempre con una pregunta o CTA suave — nunca presiones
1. Usa emojis con moderación — uno o dos por mensaje máximo
1. Cuando detectes interés real en la plataforma, invita a registrarse gratis
...
- Fecha límite ofertas: {proceso.get(‘fecha_fin_recepcion_ofertas’, ‘—’)}
  ...
  contexto_adicional = f”El cliente pregunta por {codigo_proceso}. No está analizado aún, PERO ya iniciamos el análisis automáticamente. Dile al cliente que en unos minutos tendrá el resumen del pliego.”
...
# FEATURE 1 — SCORING DE PROSPECTOS

# ═══════════════════════════════════════════════════════════════════════

def calcular_score_prospecto(perfil: dict) -> tuple[int, dict]:
“””
Calcula score 0-100 del prospecto basado en su perfil.
...
```
# Capacidad legal/administrativa (40 pts)
if perfil.get("tiene_rpe"):
    score += 20
    detalle["rpe"] = 20
if perfil.get("tiene_rnce"):
    score += 20
    detalle["rnce"] = 20

# Capacidad financiera (30 pts)
if perfil.get("tiene_estados_financieros"):
    anos = perfil.get("anos_estados_financieros", 0) or 0
    pts = min(30, anos * 15)  # 15 pts por año, máx 30
    score += pts
    detalle["estados_financieros"] = pts

# Experiencia (20 pts)
if perfil.get("ha_participado_antes"):
    score += 10
    detalle["ha_participado"] = 10
anos_exp = perfil.get("anos_experiencia", 0) or 0
if anos_exp >= 5:
    score += 10
    detalle["experiencia"] = 10
elif anos_exp >= 2:
    score += 5
    detalle["experiencia"] = 5

# Perfil completo (10 pts)
campos_completos = sum([
    bool(perfil.get("nombre_empresa")),
    bool(perfil.get("sector")),
    bool(perfil.get("tipos_proceso")),
    bool(perfil.get("instituciones_interes")),
    bool(perfil.get("provincia")),
])
pts_perfil = campos_completos * 2
score += pts_perfil
detalle["perfil_completo"] = pts_perfil

return min(score, 100), detalle
```
...
“”“Recalcula scores de todos los prospectos activos”””
try:
result = supabase_admin.table(“perfiles_prospectos”).select(”*”).execute()
for perfil in (result.data or []):
score, detalle = calcular_score_prospecto(perfil)
supabase_admin.table(“perfiles_prospectos”).update({
“score”: score,
“score_detalle”: detalle,
“score_calculado_en”: datetime.utcnow().isoformat()
}).eq(“id”, perfil[“id”]).execute()
print(f”[Closer] Scores recalculados para {len(result.data or [])} prospectos”)
except Exception as e:
print(f”[Closer] Error recalculando scores: {e}”)
...
“”“Recalcula scores de todos los prospectos — llamar desde n8n o manualmente”””
if x_agent_secret != AGENT_SECRET:
raise HTTPException(status_code=401, detail=“Unauthorized”)
...
# FEATURE 2 — RESUMEN DIARIO 6PM

# ═══════════════════════════════════════════════════════════════════════

@closer_router.post(”/resumen/run”)
async def generar_resumen_diario(
background_tasks: BackgroundTasks,
x_agent_secret: Optional[str] = Header(None)
):
“”“Cron 6pm — genera y envía resumen diario por Telegram”””
if x_agent_secret != AGENT_SECRET:
raise HTTPException(status_code=401, detail=“Unauthorized”)
...
“”“Genera resumen del día y lo envía por Telegram”””
try:
hoy = datetime.utcnow().date()
inicio_dia = datetime.utcnow().replace(hour=0, minute=0, second=0).isoformat()

```
    # Mensajes recibidos hoy
    msgs = supabase_admin.table("mensajes_closer") \
        .select("id", count="exact") \
        .eq("rol", "cliente") \
        .gte("enviado_en", inicio_dia) \
        .execute()
    total_mensajes = msgs.count or 0

    # Conversaciones nuevas hoy
    convs_nuevas = supabase_admin.table("conversaciones_closer") \
        .select("id", count="exact") \
        .gte("creado_en", inicio_dia) \
        .execute()
    nuevas = convs_nuevas.count or 0
    ...
    convs_activas = supabase_admin.table("conversaciones_closer") \
        ...
        .not_.in_("etapa", ["cerrado_ganado", "cerrado_perdido", "inactivo"]) \
        .execute()
    activas = convs_activas.count or 0

    # Señales de cierre hoy (estado hot)
    hot = supabase_admin.table("conversaciones_closer") \
        .select("id", count="exact") \
        .eq("estado", "hot") \
        .execute()
    senales = hot.count or 0

    # Top 5 prospectos por score
    top = supabase_admin.table("perfiles_prospectos") \
        .select("contact_name, score, sector, tipos_proceso, tiene_rpe, tiene_rnce") \
        .order("score", desc=True) \
        .limit(5) \
        .execute()
    top_prospectos = top.data or []

    # Construir mensaje Telegram
    top_texto = ""
    for p in top_prospectos:
        score = p.get("score", 0) or 0
        nombre = p.get("contact_name") or "Sin nombre"
        sector = p.get("sector") or "—"
        rpe = "✅" if p.get("tiene_rpe") else "❌"
        rnce = "✅" if p.get("tiene_rnce") else "❌"
        top_texto += f"\n{emoji_score(score)} <b>{nombre}</b> — {score}/100 | {sector} | RPE:{rpe} RNCE:{rnce}"

    resumen = f"""📊 <b>RESUMEN DEL DÍA — {hoy.strftime('%d/%m/%Y')}</b>
```

💬 Mensajes recibidos: <b>{total_mensajes}</b>
👥 Conversaciones nuevas: <b>{nuevas}</b>
📱 Conversaciones activas: <b>{activas}</b>
🔥 Señales de cierre: <b>{senales}</b>

<b>TOP PROSPECTOS:</b>{top_texto if top_texto else ‘\nSin prospectos calificados aún’}

<i>Resumen generado automáticamente por el Agente Closer</i>”””
...
```
    ...
    # Guardar en Supabase
    supabase_admin.table("resumenes_diarios").insert({
        "fecha": hoy.isoformat(),
        "total_mensajes_recibidos": total_mensajes,
        "conversaciones_nuevas": nuevas,
        "total_conversaciones_activas": activas,
        "senales_cierre": senales,
        "top_prospectos": top_prospectos,
        "texto_resumen": resumen
    }).execute()

    print(f"[Closer] Resumen diario enviado — {hoy}")

except Exception as e:
    print(f"[Closer] Error generando resumen: {e}")
    await enviar_telegram(f"⚠️ Error generando resumen diario: {e}")
```

# ═══════════════════════════════════════════════════════════════════════

# FEATURE 3 — ANÁLISIS DE PLIEGO ON-DEMAND

# (cuando cliente menciona proceso no analizado, lo dispara automáticamente)

# ═══════════════════════════════════════════════════════════════════════

async def disparar_analisis_pliego_bg(codigo_proceso: str, phone: str, conv_id: str):
“””
Dispara análisis de pliego con Gemini cuando un cliente pregunta por un proceso
que no está en analisis_pliego. Notifica al cliente cuando termina.
“””
try:
print(f”[Closer] Disparando análisis del pliego {codigo_proceso}…”)

```
    # Buscar el proceso en la tabla procesos para obtener URL del pliego
    resultado = supabase_admin.table("procesos") \
        .select("codigo_proceso, titulo, url_pliego, unidad_compra") \
        .eq("codigo_proceso", codigo_proceso) \
        .limit(1) \
        .execute()

    if not resultado.data:
        await enviar_whatsapp(phone, f"No encontré el proceso {codigo_proceso} en nuestra base de datos. ¿Puedes confirmar el código exacto?")
        return

    proceso = resultado.data[0]
    url_pliego = proceso.get("url_pliego")

    if not url_pliego:
        await enviar_whatsapp(phone, f"Tengo datos básicos del proceso {codigo_proceso} pero el pliego aún no está disponible para análisis. Te aviso cuando esté listo 👍")
        return

    # Notificar al cliente que estamos analizando
    await enviar_whatsapp(phone, f"Dame unos minutos, estoy analizando el pliego de {proceso.get('titulo', codigo_proceso)} 🔍")

    # Llamar al endpoint de análisis interno
    import httpx
    async with httpx.AsyncClient(timeout=120) as client:
        resp = await client.post(
            f"https://app.licitacionlab.com/api/analizar-pliego",
            json={"proceso_id": codigo_proceso, "url": url_pliego},
        ...
        if resp.status_code == 200:
            ...
            if analisis:
                ...
                await enviar_whatsapp(phone,
                    f"✅ Listo! Aquí el resumen del pliego:\n\n{resumen[:400]}...\n\nEl análisis completo está disponible en LicitacionLab 👉 app.licitacionlab.com"
                )
                # Notificar a Lonny también
                await enviar_telegram(
                    f"🔍 <b>Análisis disparado por cliente</b>\n\n"
                    f"Proceso: {codigo_proceso}\n"
                    f"Título: {proceso.get('titulo', '—')}\n"
                    f"Cliente: {phone}"
                )
        else:
            await enviar_whatsapp(phone, f"Tuve un problema analizando el pliego. El Ing. Luis Antigua lo revisará personalmente y te contactará.")

except Exception as e:
    print(f"[Closer] Error en análisis on-demand {codigo_proceso}: {e}")
    await enviar_whatsapp(phone, "Tuve un problema técnico analizando el pliego. Te aviso cuando esté listo 🙏")
```